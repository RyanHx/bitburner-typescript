...
export async function main(ns: NS): Promise<void> {
    ...
    while (true) {
        ...
        const growth_guard = maxMoney / money > 1 ? maxMoney / money : 1;
        ns.print(` grow____: ${ns.tFormat(ns.getGrowTime(server))} (t=${Math.ceil(ns.growthAnalyze(server, growth_guard))})`);
        ns.print(` weaken__: ${ns.tFormat(ns.getWeakenTime(server))} (t=${Math.ceil((sec - minSec) * 20)})`);
        await ns.sleep(<number>flags.refreshrate);
    }
}
